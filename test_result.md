...
agent_communication:
  - agent: "main"
    message: "Addition 4 backend: extended User model + OnboardingData + SplitUpdateRequest with optional training_day_indices (List[int], 0=Mon..6=Sun). Both POST /api/onboarding/complete and PUT /api/users/split now persist the field when included and preserve existing value when omitted. Please run targeted backend tests for the new field on the two endpoints listed in the task above. No other backend changes."
  - agent: "testing"
