...
test_plan:
  ...
  - task: "TRACKD AI Nutrition Coach Insights & Snooze"
    ...
    status_history:
      - working: "NA"
        agent: "main"
        comment: "Added /api/coach/insights (GET) and /api/coach/snooze (POST). Insights endpoint runs deterministic rules engine: plateau detection (cutters with weight unchanged ≤0.5kg over 14d → CalorieAudit, TDEE update, DietBreak, Refeed, ProteinCheck suggestions), same diet warning (≥5 consecutive days identical meal signatures), weekly review (Sunday-prioritized: avg kcal/protein/weight change/workouts done), cutting deficit tracker (warns if >500kcal/day deficit or low protein), bulking under-eating (3+ days under target) and bulking estimate (200-500 surplus optimal), and maintenance trending checks. Returns prioritized, snoozable insights array. Snooze endpoint upserts to coach_snoozes collection with TTL-style expires_at filter; insights endpoint excludes any snoozed IDs that are still active. Test scenarios: (1) auth required (401 without bearer); (2) /insights returns {insights:[], count:0} for fresh user; (3) /snooze persists and is honoured by subsequent /insights call."
      - working: true
        agent: "testing"
        comment: "✅ All coach endpoints verified working. (1) AUTH GATING: GET /api/coach/insights without auth → 401 ✓. POST /api/coach/snooze without auth → 401 ✓. (2) EMPTY USER: GET /insights with auth on test-user-trackd-1777237904201 (0 meals, 0 measurements) initially returned 200 {insights:[], count:0} — correct behaviour since rules engine requires meal/measurement data to fire. The review request stated the user 'has measurements/meals from earlier tests' but DB inspection showed meals and body_measurements collections were actually empty for this user (only users doc + goals present, goal_type=build_muscle, goal_calories=3009, weight_kg=80). (3) SEEDED TEST: I inserted 4 test meals in current week (2026-04-26 and 2026-04-27, calories 650/900/300/850) and re-ran. GET /insights → 200, count=1, insight[0] = {id:'weekly_20260426', type:'weekly_review', priority:50, severity:'info', title:'Weekly Review', message:'This week you averaged 1350 kcal against your 3009 kcal goal. Protein was 92g (58% of 160g target). You completed 0 workouts.', science present, data={avg_kcal:1350, target_kcal:3009, avg_protein:92, target_protein:160, weight_change_kg:0, workouts_done:0, estimated_change_kg:null}, suggestions=[{title:'Focus area: protein', body:'Hit 160g protein next week (you were 92g). Add 1 protein-forward meal per day.'}]}. All 9 required keys (id, type, priority, severity, title, message, science, data, suggestions) present. (4) SNOOZE FLOW: POST /api/coach/snooze {insight_id:'weekly_20260426', hours:1} → 200 {success:true, expires_at:'2026-04-27T22:23:35.661+00:00'} ISO timestamp. DB verified: coach_snoozes upserted with matching user_id + insight_id + expires_at. Followup GET /api/coach/insights → 200 count=0 — snoozed insight correctly filtered out. Test meals and snooze doc cleaned up post-test. 68/68 assertions passed across all 3 feature areas. NOTE to main agent: the 'expect weekly_review at minimum for fresh test user' part of the review prompt is incorrect — the rules engine is data-driven and will correctly return [] for a user with no meals/measurements. The rules engine itself is working as designed. Consider seeding sample meals during user creation if you want insights to appear by default for the demo test user."
